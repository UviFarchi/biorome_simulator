...
const columnsLeft = computed(() =>
  Array.isArray(props.data?.columnsLeft) ? props.data.columnsLeft : []
);
const headersLeft = computed(() =>
  Array.isArray(props.data?.headersLeft) ? props.data.headersLeft : []
);
function rowKey(r) {
  return r?.rowKey || (r ? `${r.row},${r.col}` : '');
}
function isOpenA(r) {
  return !!open.A[rowKey(r)];
}
function isOpenB(r) {
  return !!open.B[rowKey(r)];
}
function toggleA(r) {
  const k = rowKey(r);
  if (!k) return;
      ...
      <tbody>
        <tr v-for="r in rows" :key="rowKey(r)">
          ...
          <td class="expand-cell">
            <button class="linklike" @click.stop="toggleA(r)">
              {{ isOpenA(r) ? 'Hide ⌃' : 'Show ⌄' }}
            </button>
            <div v-if="isOpenA(r)" class="inline-expand lane-fit" @click="toggleA(r)">
              <table class="kv inner onecol lane-fit">
                <thead>
                  <tr>
                    <th class="wrap">{{ r.expandA?.title || 'Details' }}</th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  <tr v-if="!r.expandA?.groups?.length">
                    <td>—</td>
                  </tr>
                </tbody>
              </table>
          ...
          <!-- Stale details -->
          <td class="expand-cell">
            <button class="linklike" @click.stop="toggleB(r)">
              {{ isOpenB(r) ? 'Hide ⌃' : 'Show ⌄' }}
            </button>
            <div v-if="isOpenB(r)" class="inline-expand lane-fit" @click="toggleB(r)">
              <table class="kv inner onecol lane-fit">
                <thead>
                  <tr>
                    <th class="wrap">{{ r.expandB?.title || 'Details' }}</th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  <tr v-if="!r.expandB?.groups?.length">
                    <td>—</td>
                  </tr>
                </tbody>
              </table>
