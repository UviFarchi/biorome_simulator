...
<template>
  <div class="tile-info" v-if="tile">
    ...
    <MetricsTable
      :key="tileKey + ':soil'"
      title="Soil"
      :data="map.selectedTile.value?.soil || {}"
    />
    <MetricsTable
      :key="tileKey + ':topography'"
      title="Topography"
      :data="map.selectedTile.value?.topography || {}"
    />
    <MetricsTable
      :key="tileKey + ':resources'"
      title="Resources"
      :data="map.selectedTile.value?.resources || {}"
    />
