        ...
        <tr v-for="row in realRows" :key="row.instance.id || `real-${row.index}`">
          <td class="entity-cell">
            ...
            <div class="entity-visual">
              <img
                v-if="
                  getImageOrIcon(group, row.instance.type, row.instance.growthStage)?.includes('/')
                "
                :src="getImageOrIcon(group, row.instance.type, row.instance.growthStage)"
                class="entity-image"
                alt=""
              />
              <span v-else class="entity-icon">{{
                getImageOrIcon(group, row.instance.type, row.instance.growthStage)
              }}</span>
            </div>
          ...
          <td>
            <MetricsTable
              title="Properties"
              field="_biota"
              :data="biotaBlockMeasured(row.instance)"
            />
          </td>
        </tr>
      </tbody>
...
.badge--real {
  color: var(--ink);
  background: color-mix(in srgb, var(--color-background) 8%, transparent);
}
.badge--proj {
