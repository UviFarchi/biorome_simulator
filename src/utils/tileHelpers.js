...
import { measurementExpiration } from '@/dict/measurementExpiration.js';
...
function measureTileProperty(propertyObj, propertyKey) {
  ...
  const durations = measurementExpiration; // expiry durations table (see section 2)
  let expireDays = 0;
...
function getAdjacentTiles(tile, tilesGrid) {
  ...
  for (const [deltaRow, deltaCol] of directions) {
    ...
    if (
      row >= 0 &&
      row < tilesGrid.length &&
      col >= 0 &&
      col < tilesGrid[0].length &&
