...
function selectWeatherLabel( temperature, rainfall, cloudCover ) {
    console.log('Selecting Weather Label: ', temperature)
    switch (true) {
...
export default function generateWeather() {
    ...
    // 2) Apply ongoing multi-day events (today counts), then decrement and prune
    const active = game.events.weather || []
    ...
    for (const w of candidates) {
        if (Math.random() < (w.frequency ?? 0)) {
            ...
            if ((w.duration ?? 1) > 1) {
                const existing = (game.events.weather || []).find(ev => ev.id === w.id)
                if (existing) {
                    existing.remaining = Math.max(existing.remaining, (w.duration - 1))
                } else {
                    game.events.weather.push({
                        id: w.id,
                        type: 'weather',
                        headline: w.headline,
    ...
    // simple humidity proxy if events/base pushed it out
