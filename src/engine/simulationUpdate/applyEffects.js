...
export async function applyEffects() {
  ...
  try {
    ...
    const newTiles2D = tiles2D.map((row, r) =>
      row.map((orig, c) => mergeProcessedIntoOriginal(orig, processedFlat[r * width + c] || {}))
    );
    ...
    const merged2D = tiles2D.map((row, r) =>
      row.map((orig, c) => mergeProcessedIntoOriginal(orig, processed2D[r]?.[c] || {}))
    );
...
function mergeProcessedIntoOriginal(original, processed) {
  ...
  if (original.assemblies && !Array.isArray(original.assemblies)) {
    out.assemblies = {
      real: Array.isArray(processed.assemblies)
        ? processed.assemblies
        : realArray(original.assemblies),
  ...
  return tiles2D.map((row) =>
    row.map((tile) => {
      ...
      const prepared = {
        ...
        topography: Object.keys(tile.topography || {})
          .filter((k) => FX.topography?.[k])
          .map((k) => ({
            key: k,
            subject: null,
          })),
        soil: Object.keys(tile.soil || {})
          .filter((k) => FX.soil?.[k])
          .map((k) => ({ key: k, subject: null })),
        resources: Object.keys(tile.resources || {})
          .filter((k) => FX.resources?.[k])
          .map((k) => ({
            key: k,
            subject: null,
          })),
          ...
          for (const eff of effectList) {
            ...
            if (eff.target === 'animals') {
              ...
              for (const a of targets)
                if (a?.[eff.property]?.env !== undefined) {
            ...
            if (eff.target === 'plants') {
              ...
              for (const p of targets)
                if (p?.[eff.property]?.env !== undefined) {
