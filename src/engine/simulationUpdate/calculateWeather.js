...
const weatherEvents = [
  ...
  },
  {
    ...
    duration: 2,
    effect: [
      { target: 'weather', parameter: 'rainfall', delta: 1 },
...
const jitter = (mean, min, max) =>
  clamp(mean + ((Math.random() - 0.5) * (max - min)) / 3, min, max);
...
export default function calculateWeather() {
  ...
  let temperature = Math.round(
    jitter(season.temperature.mean, season.temperature.min, season.temperature.max)
  );
  ...
  let windSpeed = Math.round(
    jitter(season.windSpeed.mean, season.windSpeed.min, season.windSpeed.max)
  );
  ...
  const candidates = weatherEvents.filter(
    (w) => w.type === 'weather' && (w.season === 'any' || w.season === seasonKey)
  );
