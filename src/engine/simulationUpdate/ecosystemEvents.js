...
export default function ecosystemEvents() {
  ...
  for (const eventDef of EVENTS) {
    ...
    const catalog = eventDef.entity === 'animal' ? animalTypes : plantTypes;
    ...
    for (const seedTile of seedTiles) {
      ...
      for (let i = 0; i < spawnCount; i++) {
        ...
        if (eventDef.entity === 'animal') {
          targetTile.animals.real.push(makeInstance('animal', chosenSpec.type, stage));
        } else {
          targetTile.plants.real.push(makeInstance('plant', chosenSpec.type, stage));
        }
