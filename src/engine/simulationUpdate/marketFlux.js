...
function eventPriceModifier(game, productType) {
  ...
  for (const e of evts) {
    if (
      Array.isArray(e?.affectedTypes) &&
      e.affectedTypes.includes(productType) &&
      typeof e?.priceModifier === 'number'
    ) {
      m *= e.priceModifier;
    }
  }
...
function plantStagePrices(plants, productPrices) {
  ...
  for (const p of list) {
    ...
    const isPerennial =
      p.plantMaterialKey === 'wood' ||
      days.some((d) => (d || 0) >= 1000) ||
      stages.includes('sapling');
...
function genContractOffers(game, market, productPrices, n = 1) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const pricePerUnit = Math.max(
      1,
      Math.round(base * (1.1 + Math.random() * 0.5) * eventPriceModifier(game, k))
    );
    const qty = rint(5, 20);
...
export function marketFlux() {
  ...
  game.currentEvents.market = Array.isArray(game.currentEvents.market)
    ? game.currentEvents.market
    : [];
