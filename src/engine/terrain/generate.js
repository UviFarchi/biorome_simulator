...
function fractalBrownianMotion2D(
  x,
  y,
  seed,
  octaves = 5,
...
export default function generate() {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      const hx =
        fractalBrownianMotion2D(x + dxNormalized, y, baseSeed) -
        fractalBrownianMotion2D(x - dxNormalized, y, baseSeed);
      const hy =
        fractalBrownianMotion2D(x, y + dyNormalized, baseSeed) -
        fractalBrownianMotion2D(x, y - dyNormalized, baseSeed);
      ...
      const waterTableNoise = fractalBrownianMotion2D(
        x + 0.137,
        y + 0.291,
        baseSeed + 12345,
        4,
      ...
      // Soil physics
      {
        ...
        tile.soil.bulkDensity.env = +(1.1 + 0.5 * (1 - wetness) * p2).toFixed(2); // 1.10–1.60 g/cm3
        tile.soil.penetrationResistance.env = +(0.3 + 1.5 * (1 - wetness) * p3).toFixed(2); // 0.30–1.80 MPa
